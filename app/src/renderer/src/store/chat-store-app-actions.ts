import type { AppRoute } from './chat-store-types'
...
> {
  ...
  return {
    ...
    openInvestment: () => {
      const current = get()
      // settingsReturnRoute 只能是 'settings' 之外的 AppRoute
      const ret = current.route === 'investment' ? current.settingsReturnRoute : current.route
      const safeRoute: Exclude<AppRoute, 'settings'> =
        ret === 'settings' ? 'chat' : ret
      set({
        route: 'investment' as AppRoute,
        settingsReturnRoute: safeRoute
      })
    },
