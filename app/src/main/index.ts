...
let __activeSettings: AppSettingsV1 | undefined
export function setActiveSettings(s: AppSettingsV1 | undefined): void { __activeSettings = s }
function activeSettings(): AppSettingsV1 {
  if (__activeSettings) return __activeSettings
  // Fallback minimal settings
  return {
    version: 1, locale: 'zh', theme: 'system', uiFontScale: 'medium',
    provider: { apiKey: '', baseUrl: '', providers: [] },
    agents: {} as any, workspaceRoot: '~/.deepseekgui', log: { enabled: false, retentionDays: 30 },
    notifications: { turnComplete: false }, appBehavior: {} as any,
    keyboardShortcuts: {} as any, write: {} as any, claw: {} as any,
    schedule: {} as any, guiUpdate: { channel: 'stable' }, codePromptPrefix: ''
  }
}
...
async function stopManagedRuntimes(): Promise<void> {
  if (!managedRuntimesStopPromise) {
    managedRuntimesStopPromise = (async () => {
      ...
      clawRuntime?.stop()
      stopWeixinBridgeRuntime()
      await getActiveRuntimeAdapter(activeSettings()).stopAndWait()
    })().finally(() => {
...
async function ensureKunRuntime(settings: AppSettingsV1): Promise<void> {
  ...
  const adapter = getActiveRuntimeAdapter(activeSettings())
...
app.whenReady().then(async () => {
  ...
  if (resolveConfiguredApiKey(initial)) {
    setTimeout(() => {
      void getActiveRuntimeAdapter(initial).resolveExecutable(initial).catch((err) => {
