...
Wire the existing `@earendil-works/pi-tui` `Editor` to its built-in `CombinedAutocompleteProvider` for slash-command completion, and remove the in-house shadow state that currently duplicates it. The provider is upstream-maintained and already implements slash completion (with fuzzy filter), `@`-prefixed file completion, and `/<cmd> <arg>` parameter completion. The CLI is wired with 3 lines: `editor.setAutocompleteProvider(new CombinedAutocompleteProvider(getAllSlashCommands(), process.cwd()))`. The result is a cmd view behaviorally identical to `codex` / `claude code` (arrow keys navigate, Tab accepts without submitting, Enter accepts AND submits, Esc dismisses, type-to-filter with fuzzy, `@` triggers file completion), implemented in **3 lines of new code** and a net **-1,222 lines** of removed shadow state.
