...
#
# 强制: 管理页面只能 import Package 内部的页面/API边界，禁止
...
#
# 调用方:
#   bash scripts/lint-web-boundary.sh                  # 扫描管理 Package 页面
#   WEB_DIR=/tmp/foo bash scripts/lint-web-boundary.sh # 自定义根(测试用)
#
# 退出码:
#   0 — 边界 OK
#   1 — 至少一个管理页面文件 import 了禁模块
#
...
WEB_DIR="${WEB_DIR:-packages/pi-management/src}"

if [[ ! -d "$WEB_DIR" ]]; then
  # 没有管理页面目录时不算违规。
  echo "✓ web-boundary: $WEB_DIR 不存在(未启用管理页面), 跳过"
  exit 0
fi

# 禁 import 的业务模块前缀。任何管理页面文件出现以下 import 模式
# 即视为越界。
FORBIDDEN_PATTERNS=(
  "src/agent/"
  "src/tools/"
