 ...
 */
export interface PiSubagentService {
  /**
   * Run a subagent synchronously (blocking)
   */
  run(config: PiSubagentConfig, prompt: string, context?: SubagentContext): Promise<SubagentResult>;

  /**
   * Run a subagent asynchronously (non-blocking, returns task ID)
   */
  runAsync(config: PiSubagentConfig, prompt: string, context?: SubagentContext): Promise<string>;
...
export type SubagentRunner = PiSubagentService;
 ...
 */
export const DEFAULT_SUBAGENT_CONFIG: Record<SubagentType, Partial<PiSubagentConfig>> = {
  general: {
    maxTurns: 50,
    model: 'inherit',
...
// (analyst / sentiment / transcript) is the canonical consumer. Tests inject
// a mock `runFn` to avoid LLM calls.
