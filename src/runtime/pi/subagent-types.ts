...
export interface LoadedPiAgentSpec {
  name: string;
  description: string;
  systemPrompt: string;
  tools: string[] | '*';
...
class AgentDirectoryLoader {
  private loadedAgents: Map<string, LoadedPiAgentSpec> = new Map();
  ...
  get(name: string): LoadedPiAgentSpec | undefined {
    return this.loadedAgents.get(name);
  }

  list(): LoadedPiAgentSpec[] {
    return Array.from(this.loadedAgents.values());
  }

  listBuiltIn(): LoadedPiAgentSpec[] {
    return this.list().filter(a => a.source === 'builtin');
  }

  listFromFiles(): LoadedPiAgentSpec[] {
    return this.list().filter(a => a.source === 'file');
  }

  register(agent: LoadedPiAgentSpec): void {
...
export class EnhancedSubagentRunner extends SubagentRunner {
   ...
   */
  listBuiltInAgents(): LoadedPiAgentSpec[] {
   ...
   */
  getPiAgentSpec(name: string): LoadedPiAgentSpec | undefined {
...
export function createForkSubagentTool(): PiTool {
  return new PiTool({
    name: 'fork_subagent',
    description: FORK_SUBAGENT_DESCRIPTION,
    schema: ForkSubagentSchema,
    async func(input: z.infer<typeof ForkSubagentSchema>): Promise<string> {
...
export function createResumeAgentTool(): PiTool {
  return new PiTool({
    name: 'resume_agent',
    description: RESUME_AGENT_DESCRIPTION,
    schema: ResumeAgentSchema,
    async func(input: z.infer<typeof ResumeAgentSchema>): Promise<string> {
      const runner = getDefaultSubagentRunner() as EnhancedSubagentRunner;
...
export function createAgentMemoryTool(): PiTool {
  return new PiTool({
    name: 'agent_memory',
    description: AGENT_MEMORY_DESCRIPTION,
    schema: AgentMemorySchema,
    async func(input: z.infer<typeof AgentMemorySchema>): Promise<string> {
      const runner = getDefaultSubagentRunner() as EnhancedSubagentRunner;
...
export function createListAgentsTool(): PiTool {
  return new PiTool({
    name: 'list_agents',
    description: LIST_AGENTS_DESCRIPTION,
    schema: ListAgentsSchema,
    async func(input: z.infer<typeof ListAgentsSchema>): Promise<string> {
      const runner = getDefaultSubagentRunner() as EnhancedSubagentRunner;
...
export function createRunBuiltInAgentTool(): PiTool {
  return new PiTool({
    name: 'run_builtin_agent',
    description: RUN_BUILTIN_AGENT_DESCRIPTION,
    schema: RunBuiltInAgentSchema,
    async func(input: z.infer<typeof RunBuiltInAgentSchema>): Promise<string> {
      const runner = getDefaultSubagentRunner() as EnhancedSubagentRunner;
