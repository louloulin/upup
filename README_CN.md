...
**UpUp (涨涨)** 是一个**终端里的中文金融研究 AI 智能体**。它在命令行里把多源金融数据、估值模型、回测引擎和投研工作流串成一个闭环：你问一个问题，它会拉数据、做估值、跑回测、引用文件，最后给你一份带依据的结论。

它不是 Dexter 的中文翻译，也不是简单的"换皮"。它是在 [Dexter](https://github.com/virattt/dexter) 基础上 fork 之后，针对**中文投研场景**做了 8 个维度的实质性扩展：

1. **A 股数据栈** —— Tushare Pro + AKShare 接入
2. **50 个投资分析 Skill** —— 估值、技术、回测、行业、组合、风险全覆盖
3. **5 阶段投资工作流** —— `/invest` 把 detect → plan → execute → verify → report 串成流水线
4. **4 运行时插件系统** —— `bun` / `jiti` / `wasm` / `mcp` 沙箱可选
5. **EN + zh-CN 双语 i18n** —— 强类型 key，缺译编译/测试 fail
6. **多 Agent 协同** —— subagent 并行拉数据 + 投资 subagent 复核
7. **Session 2.0 / Permission 体系** —— 参考 Claude Code 的计划模式、Loop 恢复、停止 hook
8. **持续 8 轮 Sprint 打磨** —— 投研 Claude Code 改造 + Round 1-3 skills/plugins 整合 + 网关层
...
| **i18n（EN + zh-CN）** | `src/i18n/strings.ts`，组件 / Prompt / Skill 描述全双语 |
