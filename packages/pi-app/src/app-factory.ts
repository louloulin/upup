/**
 * Pi Native App factory — `createPiApp`, `PiApp`, `PiAppOptions` and friends.
 *
 * Lives in its own module so `default.ts` (which the rest of the workspace
 * pulls in for `bootstrapPiNativeServices`) can construct the singleton
 * without first importing from `index.ts`. `index.ts` historically both
 * defined `createPiApp` and re-exported `pi-native-cli`, which formed a
 * 4-node cycle:
 *
 *   investment -> default -> pi-native-cli -> index -> investment
 *
 * with the back-edge being `default -> index` for `createPiApp` itself.
 * Moving the factory here leaves `default.ts -> app-factory` (one-way),
 * `pi-native-cli -> default` (unchanged), and `index -> app-factory`
 * (one-way re-export), so the SCC collapses.
 */
import {
...
import type { PiSessionFactory, UpUpAgentRuntime, PiPromptPort } from '@upup/pi-runtime';
import type {
...
export type PiBackgroundRuntimePort = Pick<import('@upup/pi-session').PiBackgroundService, 'start'>;
