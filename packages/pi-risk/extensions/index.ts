...
export default function riskExtension(pi: ExtensionAPI): void {
  ...
  pi.registerTool({ name: 'calculate_kelly', label: 'Kelly Criterion', description: 'Calculate full Kelly, half-Kelly, and optional capital position sizing from historical win/loss assumptions.', parameters: kellyParameters, async execute(toolCallId, params, signal, _onUpdate, _ctx) {
  ...
  pi.registerTool({ name: 'calculate_risk_parity', label: 'Risk Parity Allocation', description: 'Calculate inverse-volatility risk parity weights and risk contributions for a bounded asset set.', parameters: riskParityParameters, async execute(toolCallId, params, signal, _onUpdate, _ctx) {
  ...
  pi.registerTool({ name: 'calculate_mean_variance', label: 'Mean Variance Optimization', description: 'Calculate a deterministic tangency-style portfolio using excess-return to variance scores and an optional correlation matrix.', parameters: meanVarianceParameters, async execute(toolCallId, params, signal, _onUpdate, _ctx) {
  ...
  pi.registerTool({ name: 'score_data_source', label: 'Score Data Source', description: 'Score a market data source from latency, freshness, coverage, accuracy, and price deviation.', parameters: sourceMetric, async execute(toolCallId, params, signal, _onUpdate, _ctx) {
  ...
  pi.registerTool({ name: 'compare_data_sources', label: 'Compare Data Sources', description: 'Rank multiple market data sources by reliability, latency, accuracy, or overall score.', parameters: sourceComparisonParameters, async execute(toolCallId, params, signal, _onUpdate, _ctx) {
  ...
  pi.registerTool({ name: 'calculate_correlation_matrix', label: 'Correlation Matrix', description: 'Calculate a deterministic Pearson correlation matrix and diversification interpretation for multiple assets.', parameters: correlationMatrixParameters, async execute(toolCallId, params, signal, _onUpdate, _ctx) {
  ...
  pi.registerTool({ name: 'calculate_correlation', label: 'Asset Correlation', description: 'Calculate Pearson correlation and strength between two historical return series.', parameters: correlationParameters, async execute(toolCallId, params, signal, _onUpdate, _ctx) {
