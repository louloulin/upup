...
function debugLog(msg: string) {
  try {
    ...
    mkdirSync(LOG_PATH.substring(0, LOG_PATH.lastIndexOf('/')) || '.', { recursive: true });
  } catch {
  ...
  try {
    ...
    /* swallow debug log failures so execution stays sandbox-friendly */
  }
}
...
export async function executeCronJob(
  ...
  // 3. Resolve model
  const model = job.payload.model ?? configRuntime.getConfiguredModelId();
  const modelProvider = job.payload.modelProvider ?? configRuntime.getConfiguredProvider();
