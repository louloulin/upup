...
import { runAgentForMessage } from '@upup/gateway/agent-runner';
import {
  evaluateSuppression,
  HEARTBEAT_OK_TOKEN,
  type SuppressionState,
} from '@upup/gateway/heartbeat/suppression';
import { assertOutboundAllowed, sendMessageWhatsApp } from '@upup/gateway/channels/whatsapp/index';
import { resolveSessionStorePath, loadSessionStore, type SessionEntry } from '@upup/gateway/sessions/store';
import { cleanMarkdownForWhatsApp } from '@upup/gateway/utils';
import { getSetting, getConfiguredModelId, getConfiguredProvider } from '@upup/utils/config';
...
export async function executeCronJob(
  ...
  // 3. Resolve model
  const model = job.payload.model ?? getConfiguredModelId();
  const modelProvider = job.payload.modelProvider ?? getConfiguredProvider();
