 ...
 * a burst on `push2.eastmoney.com` is answered with a dropped connection and
 * ~140s of host-wide failures, so quotes, klines and the realtime SSE connect
 * all serialize through the same gate. The Eastmoney-flavoured aliases keep the
 * provider readable at the call sites.
 */
export {
  createHostRequestGate as createEastmoneyGate,
  hostGateFor as eastmoneyGateFor,
  HostThrottleError as EastmoneyThrottleError,
  isHostThrottleError as isEastmoneyThrottleError,
  isSocketResetError as isEastmoneySocketReset,
  resetHostGates as resetEastmoneyGates,
  type HostRequestGate as EastmoneyGate,
  type HostRequestGateOptions as EastmoneyGateOptions,
} from '@upup/pi-observability';
