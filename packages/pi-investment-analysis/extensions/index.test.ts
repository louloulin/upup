...
describe('Pi investment-analysis extension', () => {
  ...
  test('runs stock analysis through the exact Platform agent-worker host and journals the result', async () => {
    ...
    (globalThis as typeof globalThis & { __upupPiHosts?: Map<string, unknown> }).__upupPiHosts = new Map([['@upup/pi-platform', {
      packageName: '@upup/pi-platform', packageVersion: '0.1.0', sessionId: 'session-stock', capabilities: ['agent-worker'],
      runAgentWorker: async (request: any) => ({ agentId: request.agentId, output: `worker:${request.role}`, sessionId: `session:${request.agentId}` }),
    }]]);
    investmentAnalysisExtension({ registerTool: (tool: { name: string; execute: (...args: any[]) => Promise<any> }) => tools.set(tool.name, tool) } as never);
  ...
  test('uses the exact session-scoped runtime worker and persists task journal', async () => {
    const tools = new Map<string, { execute: (...args: any[]) => Promise<any> }>();
    const entries: unknown[] = [];
    (globalThis as typeof globalThis & { __upupPiHosts?: Map<string, unknown> }).__upupPiHosts = new Map([['@upup/pi-investment-analysis', {
      packageName: '@upup/pi-investment-analysis', packageVersion: '0.1.0', sessionId: 'session-a', capabilities: ['research-worker'],
      runResearchWorker: async (request: any) => ({ role: request.role, output: `finding:${request.symbol}`, evidence: [{ source: 'fixture' }], sessionId: `worker:${request.role}` }),
    }]]);
    investmentAnalysisExtension({ registerTool: (tool: { name: string; execute: (...args: any[]) => Promise<any> }) => tools.set(tool.name, tool) } as never);
