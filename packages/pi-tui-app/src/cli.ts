...
export interface RunCliOptions {
  ...
  stream?: AgentRunnerPorts['stream'];
  runtime: TuiRuntime;
  capabilities?: TuiCommandCapabilities;
}
...
export async function runCli(options: RunCliOptions) {
  ...
  const modelSelection = new ModelSelectionController(onError, {
    ...
    saveApiKeyForProvider,
    getOllamaModels,
    promptRunner: options.runtime.promptRunner,
  }, () => {
  ...
  const sessionSelection = new SessionSelectionController(options.runtime.sessionService, () => {
    needsRenderOverlay = true;
    renderSelectionOverlay();
    tui.requestRender();
  });
  ...
  agentRunner = new AgentRunnerController(
    ...
    {
      stream: options.stream ?? (() => { throw new Error('Pi CLI requires an injected event stream'); }),
      sessionService: options.runtime.sessionService,
      sessionTracker: options.runtime.sessionTracker,
      fileHistory: {
  ...
  const executeCommandFromModule = async (name: string, args: string, context: { cwd: string; env: Record<string, string>; sessionId: string; model: string; state?: Record<string, unknown>; sessionDuration?: number; tools?: readonly { name: string; description: string }[]; capabilities?: TuiCommandCapabilities }): Promise<{ type: string; text?: string; message?: string; component?: unknown }> => {
    try {
      ...
      return result as { type: string; text?: string; message?: string; component?: unknown };
    } catch (e) {
  ...
  const handleSlashCommand = async (commandName: string, commandArgs: string = '') => {
    ...
    // Pi owns Skill expansion and execution; the CLI only submits the prompt.
    try {
    ...
    try {
      ...
      const result = await executeCommandFromModule(commandName, commandArgs, {
        ...
        model: modelSelection.model,
        state,
        tools: options.runtime.getSessionTools(agentRunner.sessionId),
  ...
  function renderSelectionOverlay() {
    ...
    if (sessionSelection.isActive()) {
      ...
      if (sState.appState === 'session_list') {
        const selector = createSessionSelectList(
          sState.sessions,
          async (sessionId) => {
    ...
    if (state.appState === 'model_select' && state.pendingProvider) {
      const selector = createModelSelectList(
        state.pendingModels,
