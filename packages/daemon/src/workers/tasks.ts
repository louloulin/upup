...
import { loadCronStore, saveCronStore } from '@upup/cron/store';
import { computeNextRunAtMs } from '@upup/cron/schedule';
import { executeCronJob } from '@upup/cron/executor';
import type { CronJob } from '@upup/cron/types';
import { getDefaultSubagentRunner } from '@upup/agent-runtime/subagent-runner';
import type { SubagentConfig } from '@upup/agent-runtime/subagent';
...
export class TasksWorker implements Worker {
  ...
  private async executeScheduledAgent(task: Task): Promise<TaskResult> {
    ...
    try {
      ...
      const { Agent } = await import('@upup/agent-runtime/agent');
