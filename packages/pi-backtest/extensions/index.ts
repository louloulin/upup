...
function registerHostTools(pi: ExtensionAPI): void {
  registerPiCapabilityHost(pi, PACKAGE, (host) => {
    if (!host?.providers?.tools?.getToolDefinitions) return;
...
export default function backtestExtension(pi: ExtensionAPI): void {
  ...
  pi.registerTool({ name: 'run_backtest', label: 'Run Historical Backtest', description: 'Run a deterministic batch backtest on historical trades and forward price bars, returning individual outcomes and an aggregate summary.', parameters: batchParameters, async execute(toolCallId, params, signal, _onUpdate, _ctx) {
    if (signal?.aborted) return { content: [{ type: 'text', text: 'run_backtest request aborted' }], isError: true, details: undefined };
  ...
  pi.registerTool({ name: 'get_backtest_summary', label: 'Backtest Summary Guidance', description: 'Explain the meaning and limitations of historical backtest summary metrics without fabricating a result.', parameters: summaryParameters, async execute(toolCallId, params, signal, _onUpdate, _ctx) {
    if (signal?.aborted) return { content: [{ type: 'text', text: 'get_backtest_summary request aborted' }], isError: true, details: undefined };
    const result = {
  ...
  pi.registerTool({ name: 'calculate_win_rate', label: 'Calculate Backtest Win Rate', description: 'Calculate win, loss, and neutral rates from historical backtest outcomes.', parameters: winRateParameters, async execute(toolCallId, params, signal, _onUpdate, _ctx) {
    if (signal?.aborted) return { content: [{ type: 'text', text: 'calculate_win_rate request aborted' }], isError: true, details: undefined };
    return nativeResult(toolCallId, 'calculate_win_rate', 'win-rate', calculateWinRate(params.outcomes, params.includeNeutral));
  } });
  pi.registerTool({ name: 'backtest_lumpsum', label: 'Fund Lump Sum Backtest', description: 'Run a historical fund lump-sum backtest on caller-provided NAV history. No network or synthetic history is used.', parameters: fundLumpSumParameters, async execute(toolCallId, params, signal, _onUpdate, _ctx) {
    if (signal?.aborted) return { content: [{ type: 'text', text: 'backtest_lumpsum request aborted' }], isError: true, details: undefined };
  ...
  pi.registerTool({ name: 'backtest_dca', label: 'Fund DCA Backtest', description: 'Run a historical fund DCA backtest on caller-provided NAV history. No network or synthetic history is used.', parameters: fundDcaParameters, async execute(toolCallId, params, signal, _onUpdate, _ctx) {
    if (signal?.aborted) return { content: [{ type: 'text', text: 'backtest_dca request aborted' }], isError: true, details: undefined };
  ...
  pi.registerTool({ name: 'backtest_threshold', label: 'Fund Threshold Backtest', description: 'Run a historical fund threshold backtest on caller-provided NAV history. No network or synthetic history is used.', parameters: fundThresholdParameters, async execute(toolCallId, params, signal, _onUpdate, _ctx) {
    if (signal?.aborted) return { content: [{ type: 'text', text: 'backtest_threshold request aborted' }], isError: true, details: undefined };
  ...
  pi.registerTool({ name: 'backtest_evaluate_trade', label: 'Backtest Evaluate Trade', description: 'Evaluate one historical investment analysis against forward daily bars with explicit stop-loss and take-profit semantics.', parameters: evaluateParameters, async execute(toolCallId, params, signal, _onUpdate, _ctx) {
    if (signal?.aborted) return { content: [{ type: 'text', text: 'backtest_evaluate_trade request aborted' }], isError: true, details: undefined };
    const config: BacktestConfig = { evalWindowDays: params.evalWindowDays, neutralBandPct: params.neutralBandPct, engineVersion: 'v1', dataQualityMode: params.dataQualityMode ?? 'strict', asOfDate: params.asOfDate, requireTradingDays: params.requireTradingDays, costModel: params.costModel as BacktestCostModel | undefined };
    const result = evaluateTrade(params as BacktestTrade & { forwardBars: DailyBar[] }, params.forwardBars as DailyBar[], config);
  ...
  pi.registerTool({ name: 'backtest_run', label: 'Run Historical Backtest', description: 'Run a deterministic batch backtest over historical trades and forward price bars.', parameters: batchParameters, async execute(toolCallId, params, signal, _onUpdate, _ctx) {
    if (signal?.aborted) return { content: [{ type: 'text', text: 'backtest_run request aborted' }], isError: true, details: undefined };
    const config: BacktestConfig = { evalWindowDays: params.evalWindowDays, neutralBandPct: params.neutralBandPct, engineVersion: 'v1', dataQualityMode: params.dataQualityMode ?? 'strict', asOfDate: params.asOfDate, requireTradingDays: params.requireTradingDays, costModel: params.costModel as BacktestCostModel | undefined };
  ...
  pi.registerTool({ name: 'backtest_win_rate', label: 'Backtest Win Rate', description: 'Calculate win, loss, and neutral rates from historical backtest outcomes.', parameters: winRateParameters, async execute(toolCallId, params, signal, _onUpdate, _ctx) {
    if (signal?.aborted) return { content: [{ type: 'text', text: 'backtest_win_rate request aborted' }], isError: true, details: undefined };
