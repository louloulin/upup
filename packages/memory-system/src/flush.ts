...
// MemoryManager is imported dynamically to avoid circular dependency with index.ts
import { CONTEXT_THRESHOLD } from '@upup/utils/tokens';
...
`.trim();
  ...
  const { MemoryManager } = await import('./index.js');
