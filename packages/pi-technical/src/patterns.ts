...
const PATTERN_DEFINITIONS: Record<CandlePatternKind, { bias: CandlePattern['bias']; description: string }> = {
  ...
  hammer: { bias: 'bullish', description: 'Small body near the top with a long lower shadow; potential reversal up' },
  'inverted-hammer': { bias: 'bullish', description: 'Small body near the bottom with a long upper shadow; potential reversal up' },
  'shooting-star': { bias: 'bearish', description: 'Small body near the bottom with a long upper shadow after an uptrend; potential reversal down' },
  ...
  marubozu: { bias: 'bullish', description: 'Candle with no upper or lower shadow; strong continuation' },
};
...
export function recognizePatterns(bars: readonly IndicatorBar[], options: { minStrength?: number } = {}): readonly CandlePattern[] {
  ...
  for (let i = 0; i < bars.length; i += 1) {
    const bar = bars[i]!;
    // Priority 1: 3-bar reversal patterns (morning/evening star) take precedence over single-bar interpretations of the latest bar
    if (i >= 2) {
      const first = bars[i - 2]!;
      const middle = bars[i - 1]!;
      const middleRange = Math.max(range(middle), 1e-9);
      const middleBodyPct = body(middle) / middleRange;
      if (middleBodyPct < 0.3) {
        if (isBearish(first) && isBullish(bar) && middle.low < first.close && middle.low < bar.open) {
        ...
        if (isBullish(first) && isBearish(bar) && middle.high > first.close && middle.high > bar.open) {
    ...
    // Priority 2: Single-bar special patterns (doji, marubozu)
    const r = range(bar);
    if (r === 0) continue;
    const b = body(bar);
    ...
    const bodyPct = b / r;
    if (bodyPct < 0.1) {
      out.push({ kind: 'doji', index: i, date: bar.date, bias: 'neutral', strength: Number((1 - bodyPct).toFixed(2)), confidence: 0.7, description: PATTERN_DEFINITIONS.doji.description });
      continue;
    }
    ...
    // Priority 3: Hammer / shooting star (extreme shadow dominates spinning-top)
    ...
    // Priority 4: 2-bar engulfing patterns
    if (i >= 1) {
      const prev = bars[i - 1]!;
