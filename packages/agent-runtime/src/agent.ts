...
// import { getTools, getToolConcurrencyMap } from '@upup/tools-registry';
import { buildSystemPrompt, loadSoulDocument, loadRulesDocument } from './prompts.js';
import { extractTextContent, hasToolCalls } from '@upup/utils/ai-message';
import { InMemoryChatHistory } from '@upup/utils/in-memory-chat-history';
import { estimateTokens, getAutoCompactThreshold, KEEP_TOOL_USES } from '@upup/utils/tokens';
import { exceedsSizeCap, persistLargeResult, buildPersistedContent } from '@upup/utils/tool-result-storage';
import { enforceResultBudget } from '@upup/utils/tool-result-budget';
import { formatUserFacingError, isContextOverflowError } from '@upup/utils/errors';
import type { AgentConfig, AgentEvent, CompactionEvent, ContextClearedEvent, MicrocompactEvent, QueueDrainEvent, StreamMode, StreamProgressEvent, TokenUsage } from './types.js';
import type { MessageQueue } from '@upup/utils/message-queue';
...
import { useContextWatchdog, useMemoryUsage, useSessionBackgrounding, useToolMetrics, useSessionRecovery } from '@upup/hooks/agent-hooks';
...
import { recordToolCallOk, recordToolCallErr } from '@upup/telemetry/integration';
import { MemoryManager } from '@upup/memory-system/index';
import { runMemoryFlush, shouldRunMemoryFlush } from '@upup/memory-system/flush';
import { createExtractionHook, type ExtractionResult } from '@upup/memory-system/extraction';
import { getObservationBuffer } from '@upup/memory-system/observation-buffer';
import { getStopHookRegistry, registerDefaultStopHooks, type StopHookContext } from '@upup/hooks/stop-hooks';
import { isFeatureEnabled } from '@upup/utils/feature-flags';
import { resolveProvider } from '@upup/llm';
import { warn, error, info, debug, perf } from '@upup/utils/logging';
import { ModelFallbackHandler, FallbackTriggeredError, isFallbackError } from './fallback.js';
import { getConfiguredModelId, getConfiguredProvider } from '@upup/utils/config';
...
export class Agent {
  ...
  static async create(config: AgentConfig = {}): Promise<Agent> {
    ...
    const { getTools, getToolConcurrencyMap } = await import('@upup/tools-registry');
  ...
  async *run(query: string, options?: AgentRunOptions): AsyncGenerator<AgentEvent> {
    ...
    try {
      const { loadUserHooks } = await import('@upup/hooks/user-hooks');
    ...
    let daemonSession: import('@upup/daemon/session').AgentSession | null = null;
    let daemonSessionManager: import('@upup/daemon/session').SessionManager | null = null;
    let existingSessionMessages: BaseMessage[] = [];

    try {
      const { getSessionManager: getDaemonSessionManager, deserializeMessage } = await import('@upup/daemon/session');
    ...
    const cleanup = async () => {
      ...
      if (daemonSession && daemonSessionManager) {
        try {
          ...
          const { serializeMessage } = await import('@upup/daemon/session');
    ...
    while (ctx.iteration < this.maxIterations) {
      ...
      if (usage?.inputTokens) {
        ...
        try {
          const { getAppState, calculateTokenCost } = await import('@upup/state');
      ...
      if (daemonSession) {
        try {
          const { getSessionManager: getDaemonSessionManager } = await import('@upup/daemon/session');
  ...
  private async *executeToolsAndCollectMessages(
    ...
    for await (const event of this.toolExecutor.executeAll(filteredResponse as AIMessage, ctx)) {
      ...
      if (event.type === 'tool_start' && event.toolCallId) {
        ...
        try {
          const { getHookExecutor } = await import('@upup/hooks/tool-hooks');
