// @ts-nocheck
/**
 * Run the canonical Pi investment workflow (detect → plan → execute → verify → report) Command Implementation
 *
...
const commandName = 'invest'

export const call = async (
  args: string,
  _context: ToolUseContext,
): Promise<LocalCommandResult> => {
  let runner: ((...a: any[]) => Promise<string | null | undefined>) | undefined
  try {
    const mod = await import('@upup/pi-investment-workflow')
    runner = mod.runInvest as typeof runner
  } catch (error) {
    return {
      type: 'text',
      value: [
  ...
  if (!trimmed) {
    return {
      ...
      value: [
        '',
        '═══════════════════════════════════════',
        `  /${commandName} — Run the canonical Pi investment workflow (detect → plan → execute → verify → report)`,
        '═══════════════════════════════════════',
        '  Usage:',
        '    /invest <TICKER> [intent] | --resume <planId> | --fast | --list',
        '',
  ...
  try {
    const text = await runner(trimmed)
