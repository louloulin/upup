...
function result(id: string, value: unknown, isError = false) {
  return { content: [{ type: 'text' as const, text: JSON.stringify(value) }], ...(isError ? { isError: true, details: undefined } : {}), details: { auditId: id, source: 'upup-pi://cache', dataFreshness: 'live' } };
}
