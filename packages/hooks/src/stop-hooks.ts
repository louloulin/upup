...
import { warn, error, info, debug } from '@upup/utils/logging';
...
export function createMemoryExtractionHook(): StopHook {
  return {
    ...
    execute: async (context) => {
      ...
      try {
        const { getObservationBuffer } = await import('@upup/memory-system');
        const { extractMemories } = await import('@upup/memory-system');
...
export function createSessionMemoryHook(): StopHook {
  return {
    ...
    execute: async (context) => {
      try {
        const { shouldUpdateSessionMemory, updateSessionMemory } = await import('@upup/memory-system');
        const config = await import('@upup/memory-system');
