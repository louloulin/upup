...
function debugLog(msg: string) {
  try {
    const logDir = upupPath('debug', 'logs');
    const logPath = upupPath('debug', 'logs', 'gateway-outbound.log');
...
function getActive(accountId?: string): ActiveListener {
  ...
  if (!first) {
    throw new Error('No active WhatsApp listener. Run upup gateway run.');
  }
  return first;
}
