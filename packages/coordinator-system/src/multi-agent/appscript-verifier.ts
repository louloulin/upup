...
export class AppScriptVerifier {
  ...
  private async verifySkillSystem(): Promise<void> {
    try {
      const { getAllSpecializedSkills } = await import('@upup/tools-registry/skills/bundled/index');
  ...
  private async verifyInvestmentSkills(): Promise<void> {
    try {
      const { getPhase3Skills, getPhase4Skills } = await import('@upup/tools-registry/skills/bundled/index');
