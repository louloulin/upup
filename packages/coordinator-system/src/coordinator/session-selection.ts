...
import { getSessionSummaries, filterResumableSessions } from '@upup/session-system/storage';
...
export class SessionSelectionController {
  ...
  async confirmDelete(): Promise<void> {
    ...
    const { deleteSession } = await import('@upup/session-system/storage');
  ...
  async submitRename(newTitle: string): Promise<void> {
    ...
    const { renameSession } = await import('@upup/session-system/storage');
  ...
  async submitTag(tag: string): Promise<void> {
    if (!this.pendingSessionIdValue) return;
    const { tagSession } = await import('@upup/session-system/storage');
