...
import { recordFileHistorySnapshot, getFileHistoryManager } from '@upup/storage/file-history';
import { renderMessages, type RenderableMessage } from '@upup/session-system/render/index';
import { getTimeoutForTool } from '@upup/services-core/permissions/index';
...
export class AgentRunnerController {
  ...
  async resumeFromSession(sessionId: string, fork: boolean = false): Promise<string> {
    ...
    if (fork) {
      const { forkSession } = await import('@upup/session-system/storage');
    ...
    const { loadSessionForResume, processResumedConversation } = await import('@upup/session-system/restore');
