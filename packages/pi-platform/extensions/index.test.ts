  ...
  function loadTools(worker = false) {
    ...
    const dispose = publishPiCapabilityHosts(events, 'platform-session', new Map([
      ['@upup/pi-platform', {
        ...
        providers: { tools: { getToolDefinitions: () => [], getToolMetadata: () => [], getSkillDefinitions: () => [] }, ...(worker ? { workers: { runAgentWorker: async (request: { agentId: string; name: string; role: string; prompt: string; tools: readonly string[] | '*'; model?: string }, _signal: AbortSignal) => ({ agentId: request.agentId, output: `worker:${request.prompt}`, sessionId: `session:${request.agentId}` }) } } : {}) },
      }],
    ]));
    platformExtension({ events, registerTool: (tool: { name: string; execute: (...args: never[]) => Promise<{ content: readonly { text: string }[] }> }) => registered.set(tool.name, tool) } as never);
    dispose();
    return registered;
  }
