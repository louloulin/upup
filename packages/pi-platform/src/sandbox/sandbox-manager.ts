...
export class SandboxManager {
  ...
  setConfig(config: Partial<SandboxConfig>): void {
    ...
    getLogger().debug('tools', `[sandbox] Config updated: mode=${this.config.mode}, enabled=${this.config.enabled}, autoAllow=${this.config.autoAllowBash}`);
    if (oldMode !== this.config.mode) {
      getLogger().info('tools', `[sandbox] Sandbox mode changed: ${oldMode} → ${this.config.mode}`);
    }
  }
  ...
  reset(): void {
    this.config = loadSandboxConfig();
    getLogger().debug('tools', '[sandbox] Configuration reset to defaults');
  }
...
registerPiRuntimePort('platform.sandbox',{
  getStatus: () => {
