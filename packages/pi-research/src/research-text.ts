...
export function formatResearchToolResult(data: unknown): string {
...
export interface AnalyzeSentimentInput {
  ...
  readonly useDeepAnalysis?: boolean;
}

export function analyzeSentimentToolResult(input: AnalyzeSentimentInput): string {
  if (input.useDeepAnalysis) {
    const result = deepSentimentAnalysis(input.text, input.symbol);
    const sentimentIcon = { bullish: '📈', bearish: '📉', neutral: '➖' }[result.sentiment];
    const riskIcon = { low: '🟢', medium: '🟡', high: '🔴' }[result.riskLevel];
    return formatResearchToolResult({
      ...
      message: [
        ...
        `Confidence: ${(result.confidence * 100).toFixed(0)}%`,
        `Reasoning: ${result.reasoning}`,
        result.keyFactors.length > 0 ? `Key Factors: ${result.keyFactors.join(', ')}` : null,
        result.sentences.some((sentence) => sentence.negated)
          ? `[!] Negation detected in ${result.sentences.filter((sentence) => sentence.negated).length} sentence(s)`
          : null,
      ].filter(Boolean).join('\n'),
    });
  }
  const result = quickSentimentScan(input.text);
  const sentimentIcon = { positive: '📈', negative: '📉', neutral: '➖' }[result.label];
  return formatResearchToolResult({
    type: 'Quick Sentiment Scan',
    ...
    message: [
      ...
      result.keywords.length > 0 ? `Keywords: ${result.keywords.slice(0, 5).join(', ')}` : null,
      'Hint: set useDeepAnalysis=true for investment-grade analysis',
    ].filter(Boolean).join('\n'),
  });
}

export function detectEventsToolResult(text: string): string {
  const events = detectEvents(text);
  if (events.length === 0) return formatResearchToolResult({ type: 'Event Detection', count: 0, message: 'No significant investment events detected.' });
  const majorEvents = events.filter((event) => event.severity === 'major');
  const minorEvents = events.filter((event) => event.severity === 'minor');
  const eventList = events.map((event) => {
    const badge = event.severity === 'major' ? '[MAJOR]' : '[minor]';
    const values = event.values.length > 0 ? ` → ${event.values.join(', ')}` : '';
    return `  ${badge} ${event.description} (${(event.confidence * 100).toFixed(0)}% confidence)${values}`;
  }).join('\n');
  return formatResearchToolResult({
    type: 'Event Detection', count: events.length, majorCount: majorEvents.length, minorCount: minorEvents.length,
    events: events.map((event) => ({ type: event.type, description: event.description, confidence: event.confidence, severity: event.severity, values: event.values })),
    message: [`Detected ${events.length} event(s) [${majorEvents.length} major, ${minorEvents.length} minor]:`, eventList].join('\n'),
  });
}

export function extractEntitiesToolResult(text: string): string {
  const entities = extractEntities(text);
  const stocksList = entities.stocks.length > 0 ? `US Tickers: ${entities.stocks.slice(0, 10).join(', ')}` : 'US Tickers: None found';
  const chineseStocksList = entities.chineseStocks.length > 0 ? `Chinese Stocks: ${entities.chineseStocks.join(', ')}` : 'Chinese Stocks: None found';
  const numbersList = entities.numbers.length > 0 ? `Numbers: ${entities.numbers.map((number) => number.raw).join(', ')}` : 'Numbers: None found';
  const percentagesList = entities.percentages.length > 0 ? `Percentages: ${entities.percentages.join(', ')}` : null;
  const periodsList = entities.periods.length > 0 ? `Periods: ${entities.periods.join(', ')}` : null;
  const datesList = entities.dates.length > 0 ? `Dates: ${entities.dates.slice(0, 5).join(', ')}` : 'Dates: None found';
  const lines = [stocksList, chineseStocksList, numbersList, percentagesList, periodsList, datesList].filter(Boolean) as string[];
  return formatResearchToolResult({
    type: 'Entity Extraction', stocks: entities.stocks.slice(0, 10), chineseStocks: entities.chineseStocks,
    numbers: entities.numbers.map((number) => number.raw), normalizedNumbers: entities.numbers.filter((number) => number.normalized !== undefined),
    percentages: entities.percentages, periods: entities.periods, dates: entities.dates.slice(0, 5), message: `Extracted:\n  ${lines.join('\n  ')}`,
  });
}
